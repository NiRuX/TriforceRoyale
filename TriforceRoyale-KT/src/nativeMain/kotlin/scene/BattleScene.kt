 ...
 * @author frankie
 */
class BattleScene(game: Game, entrance: String?) :
    ZeldaScene(game, "images/battle-dark.png", "BattleScene") {
    private val leftTreeline: Polygon
    private val RightTreeline: Polygon
    private val deadTree: Polygon
    private val warpExit: Rectangle = Rectangle(232, 458, 16, 16)
    ...
    init {
        ...
        solids.add(deadTree)
        gameObjects.add(Warp(game, 232, 458))

        // add Link
        gameObjects.add(game.getLink())
        gameObjects.add(BlueSoldier(game, 330, 145, Direction.LEFT, 10))
        gameObjects.add(BlueSoldier(game, 150, 340, Direction.DOWN, 10))
        gameObjects.add(BlueSoldier(game, 135, 135, Direction.RIGHT, 10))
        gameObjects.add(BlueSoldier(game, 280, 400, Direction.LEFT, 20))
        gameObjects.add(GhostSoldier(game, 240, 98, Direction.DOWN))
    ...
    fun handleSwitchScene(exit: Rectangle) {
        if (exit === warpExit) {
            game.setScene(ForrestScene(game, "BattleScene"))
        }
    }
